# Diseño y Funcionamiento del Proyecto Hoyte's Bank

En este documento explico cómo está armada la plataforma bancaria **Hoyte's Bank**, cómo se conectan las páginas entre sí y cómo funciona la lógica de programación en cada archivo.
...
### Tecnologías y Estética utilizadas:
* **Estructura HTML:** Uso etiquetas estándar (`header`, `main`, `section`, `footer`, `form`, `table`).
* **Estilos CSS & Diseño Luxury:** Bootstrap 5.3.8 para organizar filas y columnas, combinado con estilos aristocráticos en `assets/css/style.css` inspirados en la herencia de **Coutts Private Banking**. Incluye fuentes clásicas (`Cormorant Garamond` y `Outfit`), el emblema heráldico de las **Tres Coronas (`♛ ♛ ♛`)** y tarjetas con gradiente oro en capas.
* **Programación:** JavaScript clásico (Vanilla JS) y jQuery 3.7.1 para manejar eventos de clics, modales interactivos (como recuperación de clave) y animaciones sencillas.
...
| :--- | :--- | :--- |
| `usuarios` | Arreglo de objetos | Guarda la lista de cuentas registradas con su nombre, usuario y contraseña. También se actualiza aquí al restablecer la contraseña. |
...
Es la pantalla de bienvenida con estética imperial de banca privada. Tiene un contenedor central con el emblema de tres coronas, título en gradiente oro y botones que llevan a `login.html` (para entrar) o a `register.html` (para crear cuenta).
...
* **En el Login (`login.html`):**
  ...
  * **Recuperación de Contraseña (`¿Olvidaste tu contraseña?`):** Incluye un modal dinámico. Al ingresar el usuario, el script busca en `localStorage.getItem("usuarios")`. Si encuentra la cuenta, revela el campo de nueva contraseña en el mismo modal y, al enviar, sobrescribe y actualiza el arreglo en `localStorage` de forma inmediata.

### 3. Menú principal (`menu.html` y vistas privadas)
* **Validación de sesión y Session Guard:** Lo primero que hace el script es revisar si existe un `usuarioLogueado`. Si no existe, manda al usuario de vuelta a `login.html`. Asimismo, las páginas públicas (`index.html`, `login.html`, `register.html`) redirigen al menú si detectan una sesión activa.
* **Auto-Logout por Inactividad (1 minuto):** En todas las páginas privadas se ejecuta un temporizador (`setTimeout`) de 60,000 ms que supervisa eventos (`mousemove`, `keypress`, `scroll`, `touchstart`). Si transcurre 1 minuto sin interacción, el sistema borra automáticamente `usuarioLogueado` de `localStorage` y regresa al login con una alerta de seguridad.
* **Tarjeta de saldo (Hoyte's E-Wallet):** Muestra en pantalla la tarjeta virtual de banca privada con el saldo formateado en Libras Esterlinas (£ / `en-GB`), la divisa e identificación institucional **HOYTE'S E-WALLET** y el nombre del titular.
